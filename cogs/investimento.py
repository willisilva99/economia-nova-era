...
class Investimento(commands.Cog):
    ...
    async def investir(self, ctx, valor: int):
        ...
        chance_perda = random.randint(1, 100)  # Chance de perda de 100
        if chance_perda <= 50:  # 50% de chance de perder moedas
            perda = random.randint(10, valor // 2)  # Perda aleatória de até metade do investimento
            ...
            ganho = random.randint(10, valor)  # Ganho aleatório
    ...
    async def cancelar_investimento(self, ctx):
        ...
        penalidade = random.randint(5, valor_investido // 4)  # Penalidade de até 25%
        valor_recuperado = valor_investido - penalidade
        update_saldo(ctx.author.id, valor_recuperado)  # Recupera o valor menos a penalidade
        del self.investimentos[ctx.author.id]  # Remove o investimento
